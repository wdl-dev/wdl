...
- Required Workflow step claim, register-wait, and replay terminal variants to carry their own output or error field, persisted backend-owned operation kinds so legacy or mismatched replay falls back to the authoritative endpoint, and treated malformed authoritative responses as result-unknown while preserving explicit JSON null payloads.
- Bounded Durable Object alarm mutation and delivery responses to 16 KiB and required exact success variants before SQLite compensation decisions or destructive Workflows job finalization.
...
- Added a 32 MiB per-task wire-byte admission budget for in-flight Runtime KV responses. Valid declared lengths fill one allocation, full-span `arrayBuffer` reads reuse it, excess reads cancel upstream and fail closed, JSON/Base64 construction remains inside the lease, and a 5-second deadline releases abandoned work. Batch responses must exactly match requested keys before values are decoded one at a time. Private entrypoint props plus `withEnv()` async context keep cached Workflow KV facades and reverse-JSRPC step callbacks invocation-scoped; affected step results are not durably committed, while unscoped or expired ids cannot affect another invocation.
...
- **Deployment:** Workflow step kinds require the schema-3 maintenance reset documented in `docs/modules/workflows.md`: roll and drain user-runtime/system-runtime readers first, quiesce dispatch, stop Scheduler, scale Workflows to zero, clear the dedicated Workflows DB 2 including its schema marker, then start the new Workflows/Scheduler without old/new Workflows overlap. Do not reset while any Workflow instance state (including retained terminal results or step history) or Durable Object alarm state must be preserved. Legacy non-DB2 configurations must remove that override; old runtime state is not migrated, shared databases must not be cleared, and the selected endpoint must provide an empty dedicated DB 2.
