...
- Reduced Redis latency with shared `TCP_NODELAY`, two managers per redis-proxy logical pool, concurrent connection initialization, and fewer KV key allocations.
- Added bounded Runtime KV admission, response envelopes, and deadlines; restored native scalar-read performance, tightened batch validation, and limited Workflow retries to direct host failures escaping reviewed boundaries. Dynamic Workers now reject `no_rpc`.
