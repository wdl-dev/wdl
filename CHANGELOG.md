...
- Added opt-in Durable Object restart rollouts through the Control API's `durableObjectRollout: "restart"`: promotion atomically advances the permanent restart sequence and commits the active projection, retained-version alarms resolve to the active version while the latest projection remains `restart`, and affected Gateway-proxied WebSockets close with `1012`; do-runtime rejects superseded dispatch and lazily aborts stale native facets on their next dispatch without deleting SQLite state. The default remains `preserve`, and a later preserve promotion supersedes restart work that has not yet been observed without undoing closures or facet aborts that already occurred.
- Restart rollout uses Redis projection state as its durable authority and `do-rollout:restart` only to request prompt per-worker Gateway reconciliation; successful whole-worker deletion publishes the separate exact `worker:delete` hint, while route invalidations remain cache-only and subscriber reconnect reconciles all local groups. Delete hints remain non-durable and do not carry an incarnation fence across same-name recreation. Request-owned lifecycle signals return closure work to each WebSocket's IoContext, lifecycle commands use a socket-closing deadline, transient Redis failures retry only affected groups without dropping healthy connections, and owner-local restart mode/sequence never enter invoke/connect wire payloads. Non-Gateway facets converge on their next dispatch without a global owner scan, Control-to-DO binding, or post-commit fanout protocol; whole-worker cleanup removes residual active projections while retaining the permanent sequence. Deploy Gateway, Workflows, and do-runtime projection readers before system-runtime/Control writers, pause Control mutations while that writer tier rolls, and allow API clients to send the new policy only after mutations resume.
- Gateway WebSocket reconnect now revalidates the active route before admitting a replacement backend: a healthy old-version connection may drain, but a version already inactive at that lifecycle snapshot is not reloaded; Gateway closes with `1012` so the client performs a fresh upgrade to the active version.
