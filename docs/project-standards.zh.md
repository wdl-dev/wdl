...
Redis round trip、原子性和数据边界是三项独立的设计决策：

- 只有当多个独立操作属于同一个 owner、connection lifetime、failure boundary 和 budget 时才批处理。优先使用 typed、operation-specific 的 snapshot 或 batch helper，不要暴露 generic public pipeline API。必须保持结果位置对齐和既有 item、byte、page 上限；减少一次 round trip 不能成为引入无界读取的理由。
- Pipeline 只是 transport 优化，不是原子性边界。当某次读取决定并发写入时，应使用 owner 控制的 Lua script、WATCH/MULTI/EXEC 窗口或 exact-value CAS。受保护的读取和 commit-time revalidation 必须留在该 owner 的并发边界内。
- Exact-value CAS 比较的是持久化字节，不是 parse 后的值。IFEQ/DELIFEQ 一类操作必须保留原始 bulk response bytes，并另行 parse 出读取视图；不要重新序列化 JSON 或文本来构造 expected value。
- Retry 应留在能够判断 replay 是否安全的层。不能因为某条 command 出现 script-cache miss 或 transient failure，就重放可能已经部分执行的 mixed pipeline。
- 新增 command 或 batch surface 时，应补聚焦的 wire/order 测试。Unit test 使用 fake Redis、integration 使用真实 Valkey 时，应为相关 command 语义增加共享 conformance case。WATCH conflict、script-cache loss 等恢复行为属于合同时，应补定向 fault-path integration。

测试应保护这些语义，不应强制源码永远使用某个 helper 或 pipeline 写法。语言特定的 connection 和 Lua 规则分别见 `docs/workerd-js-standards.zh.md` 与 `docs/rust-sidecar-standards.zh.md`。
