...
| Workflow deletion preflight | `tests/fixtures/workflow-lifecycle-check.json` owns the request and complete/blocked/continuation response variants. Rust owns bounded scan/member work; Control owns bounded continuation, response reads, and delete-lock renewal. An incomplete scan is never permission to delete. |
