...
| Workflow payload and list limits | `tests/fixtures/workflow-limits.json` owns params/result/request byte limits, JSON grammar bounds, create-batch count, the payload-limit error code, and the aggregate instance-list response cap. Rust enforces params/result validation and the page budget; Control shares the reader cap, while Runtime consumes request/result limits. |
| Workflow deletion preflight | `tests/fixtures/workflow-lifecycle-check.json` owns the request and complete/blocked/continuation/rescan-required response variants. Rust owns bounded scan/member work; Control owns bounded continuation, response reads, and delete-lock renewal. An incomplete scan is never permission to delete; rescan-required returns the retryable incomplete error. |
| Workflow schema-2 step migration | `tests/fixtures/workflow-schema2-steps.json` owns legacy record shapes and expected schema-3 kinds, consumed by the Rust offline converter and real-service migration tests. Runtime never infers missing kinds. |
...
| Durable Object alarm responses | `tests/fixtures/do-alarm-response.json` owns the 16 KiB cap, 5-second body deadline, exact actor, dispatch, set/delete, and cleanup success variants, failed/result-unknown dispatch errors, and the pre-mutation 4xx versus result-unknown 5xx mutation classes consumed by JavaScript and Rust; `shared/do-alarm-response.js`, `do-runtime/alarm.js`, and `rust/workflows/src/api/do_alarms/` own their runtime readers and producer. |
