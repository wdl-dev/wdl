...
| `runtime/do-client.js`、`runtime/_wdl-do-scoped-request.js`、`runtime/bindings/do.js`、`runtime/_wdl-do-transport.js`、`do-runtime/protocol/wire-grammar.js` | Tenant DO namespace/id/stub facade 及其 binding-scoped request envelope；host-only identity attachment、canonical owner-shard hint projection、owner routing、invoke/connect framing、retry 和 response stripping。 |
...
| `shared/d1-*.js`、`shared/sql-splitter.js` | Runtime、d1-runtime、control 和 tests 共用的 D1 parameter、data-field、transport、timeout、query-wire/owner-hint/result-header 与 ownership-code contract，以及 SQL splitting utilities。 |
