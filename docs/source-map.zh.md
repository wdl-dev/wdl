...
| `gateway/websocket.js` | 本地 `WebSocketPair` 终结、reconnect forwarding、`101` upgrade preservation、内部 backend reconnect-policy enforcement 和 session-policy lifecycle closure。 |
...
| `gateway/websocket-lifecycle.js` | 进程内 WebSocket lifecycle admission、registry/reconciliation 和 atomic route/session-policy reads。 |
...
| `runtime/dispatch.js`、`runtime/dispatch/*` 和 `runtime/infrastructure-error.js` | Fetch、scheduled、queue 与 workflow dispatch，workflow step facade、replay cache、deterministic JSON helper、host-owned KV read failure code、facade RPC trampoline method 与 reporter origin，以及 report-only Workflow failure latch。 |
| `runtime/load.js` 和 `runtime/load/*` | Bundle decode、module rewrite、env construction、wrapper generation、runtime 注入源码 ownership、hidden binding stripping，以及与 Control 共享的 root `_wdl-*` reserved-module namespace。`module-rewrite.js` 拥有 private Workflow infrastructure reporter prop；`wrapper-generate.js` 拥有 tenant-inaccessible KV/reporter callable binding、原生 `RpcPromise` settlement、显式 Workflow step facade、exact-Error branding 与 escape-boundary reporting；`code-budget.js`、`injection-sources.js` 和 `wrapper-generate.js` 还负责让只声明 AI 的 bundle 生成并计入 AI facade。 |
| `runtime/load/injection-sources.js` | 三份 runtime 基配置通过 Cap'n Proto text module 嵌入的 canonical tenant-source registry。它把 `runtime/d1-client.js`、`shared/d1-data-field.js`、`shared/d1-params.js`、`shared/utf8.js`、`shared/sql-splitter.js`、`runtime/r2-client.js`、`runtime/r2-utils.js`、`runtime/do-client.js`、`runtime/_wdl-do-scoped-request.js`、`runtime/_wdl-request-id.js`、`runtime/workflows-client.js` 和 `runtime/ai-client.js` 作为 generated wrapper 输入；host-only D1 compatibility decoding 和 DO routing 不属于该 registry。 |
| `runtime/bindings/` | KV、D1、R2、Durable Objects、Workflows、ASSETS、service、queue 和 AI 的 host-side binding adapters。`runtime/bindings/kv-capacity.js` 负责进程内 KV response wire-byte admission、deadline 和 scrape-time gauge；`runtime/bindings/do.js` 与 `runtime/bindings/workflow.js` 负责 binding-scoped identity/authenticated transport；`runtime/bindings/ai.js` 负责 AI admission、provider 解析、public-only forwarding 和协议编排；`runtime/bindings/ai-capacity.js` 负责进程内 pool 计数、lease transfer、metrics、deadline 和 watchdog cleanup；`runtime/bindings/ai-provider.js` 负责 official destination、provider request policy、携带 credential 的 header 和有界 response header；`runtime/bindings/ai-sse.js` 负责增量 SSE framing 和 terminal-event classification；`runtime/bindings/ai-websocket.js` 负责有界 frame forwarding、model rewrite、idle detection 和 close propagation。 |
| `runtime/do-client.js`、`runtime/_wdl-do-scoped-request.js`、`runtime/bindings/do.js`、`runtime/_wdl-do-transport.js`、`do-runtime/protocol/wire-grammar.js` | Tenant DO namespace/id/stub facade 及其 binding-scoped request envelope；host-only identity attachment、canonical forwarding 与 owner-shard header、owner routing、invoke/connect framing、retry 和 response stripping。 |
| `runtime/ai-client.js` | Tenant realm 的 AI facade source，负责 `run()`、`models()`、OpenAI-compatible fetch、streaming 和 WebSocket helper。 |
| `runtime/bindings/workflow.js`、`runtime/workflows-client.js`、`runtime/dispatch/workflow-*.js`、`runtime/load/wrapper-generate.js` | Workflow binding-scoped identity enforcement 与 transport、facade materialization、dispatch facade、replay cache 和 step semantics。 |
...
| `shared/do-alarm-response.js` | do-runtime alarm delivery/mutation 与 Control whole-worker cleanup 共用的 5 秒 body deadline、16 KiB strict UTF-8 reader 和 exact success-variant parser。 |
...
| `rust/workflows/` | Workflows service、DB 2 state machine、internal DO alarm jobs 和 operator schema-3 migration command。`schema_migration.rs` 拥有离线 coordination、COPY/验证与显式 archive 删除；`api/schema2.rs` 拥有 instance 转换，`api/execution/schema2.rs` 拥有旧 step-kind 转换。 |
