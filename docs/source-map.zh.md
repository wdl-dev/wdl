...
| `runtime/dispatch.js`、`runtime/dispatch/*` 和 `runtime/infrastructure-error.js` | Fetch、scheduled、queue 与 workflow dispatch，workflow step facade、replay cache、deterministic JSON helper，以及用于阻止 retryable infrastructure failure 进入 durable step result 的精确 invocation-scoped host-failure attribution。 |
| `runtime/load.js` 和 `runtime/load/*` | Bundle decode、module rewrite、env construction、wrapper generation、runtime 注入源码 ownership、hidden binding stripping，以及与 Control 共享的 root `_wdl-*` reserved-module namespace。`module-rewrite.js` 拥有 private Workflow infrastructure invocation prop；`code-budget.js`、`injection-sources.js` 和 `wrapper-generate.js` 还负责让只声明 AI 的 bundle 生成并计入 AI facade。 |
...
| `shared/do-alarm-response.js` | do-runtime alarm delivery/mutation 与 Control whole-worker cleanup 共用的 16 KiB strict UTF-8 reader 和 exact success-variant parser。 |
