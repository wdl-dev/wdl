...
| 字段 | WDL 行为 |
|---|---|
| `name`、`main`、`compatibility_date`、`compatibility_flags` | 存入 immutable bundle metadata。Control 会在 commit 前拒绝格式错误、未来日期或当前 bundled workerd 不支持的 `compatibility_date`；包含 runtime 注入模块后的最终 WorkerCode 必须落在 workerd 64 MiB `workerLoader` code limit 内。 |
