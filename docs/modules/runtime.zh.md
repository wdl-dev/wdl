...
- Python Workers modules 不受支持。上游 `python_workers_20260610` flag 已不再是 experimental，也不再按 compatibility date 隐式启用；`python_workers_20260817` 仍是 experimental。Control 会拒绝新的 `py` module manifest，runtime/do-runtime 也会拒绝 retained metadata 中的 `py` module，而不是让 workerd 在 cold load 时 bootstrap Pyodide。
