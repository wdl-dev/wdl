...
- WebSocket backend reconnect 在现有有序 send queue 中保留 client message，每条连接同时受配置的 message count 和固定 32 MiB payload-byte budget 约束。Binary input 使用 byte length；字符串先检查低成本 code-unit 下界，可能放得下时再计算精确 UTF-8 字节数。入队前取得额度，send 完成或终止关闭时恰好释放一次；关闭还会立即清除排队 payload 引用，不等待未完成的 reconnect。任一上限触发时，两端以 `1013 websocket send buffer full` 关闭。这约束 Gateway 的 JavaScript queue，不是 native socket buffer 或所有 session 的进程总内存上限。
...
- Initial backend `101` 可以为无法重建应用状态的 session 设置内部响应头 `x-wdl-websocket-reconnect-policy: disabled`。Gateway 会从公开响应中删除该 header，并在 backend 丢失时以 `1012 service restart` 结束 public session，而不是发起 replacement upgrade。AI Responses 与 Realtime upgrade 会设置此策略；若 tenant 代码用独立 `WebSocketPair` 桥接 AI socket，必须把 AI upgrade response headers 复制到自己返回的 `101`，内部 policy metadata 不会自动跨越这层 response boundary。普通 Worker 和 Durable Object WebSocket 仍保留有界透明重连。
- `1012` 关闭会结束应用会话；client 需要重连并重新执行应用握手。终止关闭会丢弃排队的 client message，没有逐帧 ack/nack。
