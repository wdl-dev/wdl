...
| `POST` | `/ns/<ns>/worker/<name>/deploy` | 从 shorthand code 或完整 module manifest 创建新的 immutable version；routes、`workersDev`、`durableObjectRollout`、crons、queue consumers、service refs、platform refs、assets、vars、bindings 和 `exports` 都是 version metadata，201 响应会回显解析后的 routing/rollout policy。DO rollout 接受 `preserve` 或 `restart`，默认是 `preserve`，且 `restart` 要求 DO binding。Route host 会在 commit 前 normalize 成 canonical ASCII DNS hostname，并拒绝 URL-authority delimiter 或非 canonical IPv4 写法。Python modules 和上游 experimental compatibility flags 会在 commit 前被拒绝。 |
| `POST` | `/ns/<ns>/worker/<name>/promote` | 通过 WATCH/MULTI routing path promote `{"version":"vN"}`。成功响应会返回 `platformDomain`、`workersDev`、`durableObjectRollout` 和 `restartSequence`，同时以启用时的 `urls.platform` 和 `urls.routes[]` 中每条 active pattern 对应的 URL hint 返回 canonical URL hint；每条 hint 按 operator 原始 pattern 拼出，因此 prefix route 会保留尾部 `*`。Route、active rollout projection、永久 restart sequence 和 restart publication 会原子提交。Host declaration 失败是 403；live pattern conflict 是 409；transaction contention 耗尽是 503。 |
...
| `worker:do-rollout:<ns>:<worker>` | String | Control | 与 route flip 同事务提交的 active DO rollout `{version,mode,restartSequence}` projection。 | Whole-worker delete 删除；缺失 state 默认 `preserve`。 |
| `worker:do-rollout-seq:<ns>:<worker>` | String counter | Control | 永久 monotonic DO restart-event allocator。 | Whole-worker delete 后仍保留，确保重建后的下一次 restart 不会复用 stale Gateway session 已观察过的 sequence。 |
...
- 支持 DO restart rollout 时，先部署 Gateway、Workflows 和 do-runtime projection reader，再部署 system-runtime/Control writer。system-runtime 滚动期间暂停 Control mutations，待该 tier 稳定后再恢复，并且只能在此后允许 API client 发送新 bundle policy。
