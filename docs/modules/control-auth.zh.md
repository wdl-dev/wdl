...
Control 是静态控制面 worker，负责 deploy、promote、lifecycle、secret、route、D1、R2、AI、workflow、log-tail 和 auth-token API。Auth 是静态 JSRPC worker，负责校验和签发 scoped admin token。
...
| `POST` | `/ns/<ns>/worker/<name>/deploy` | 从 shorthand code 或完整 module manifest 创建新的 immutable version；routes、`workersDev`、`sessionPolicy`、crons、queue consumers、service refs、platform refs、assets、vars、bindings 和 `exports` 都是 version metadata，201 响应会回显解析后的 routing/session policy。`sessionPolicy` 接受 `preserve` 或 `restart`，默认是 `preserve`。Route host 会在 commit 前 normalize 成 canonical ASCII DNS hostname，并拒绝 URL-authority delimiter 或非 canonical IPv4 写法。Python modules 和上游 experimental compatibility flags 会在 commit 前被拒绝。 |
| `POST` | `/ns/<ns>/worker/<name>/promote` | 通过 WATCH/MULTI routing path promote `{"version":"vN"}`。成功响应会返回 `platformDomain`、`workersDev`、`sessionPolicy` 和 `restartSequence`，同时以启用时的 `urls.platform` 和 `urls.routes[]` 中每条 active pattern 对应的 URL hint 返回 canonical URL hint；每条 hint 按 operator 原始 pattern 拼出，因此 prefix route 会保留尾部 `*`。Route、active session policy projection、永久 restart sequence 和 restart publication 会原子提交。Host declaration 失败是 403；live pattern conflict 是 409；transaction contention 耗尽是 503。 |
...
| `GET` | `/ns/<ns>/ai/providers[/<name>]` | 列出 provider metadata 与 credential-present 状态，不返回 credential value。 |
| `PUT` / `DELETE` | `/ns/<ns>/ai/providers/<name>` | 使用新 revision 替换 canonical provider metadata。同 kind 更新会保留 credential；kind 变化或在 credential-only residue 上创建 metadata 时会清除 credential。Delete 会一起删除 metadata 与 credential。 |
| `PUT` | `/ns/<ns>/ai/providers/<name>/credential` | 在精确 provider-revision CAS 下加密 credential。 |
| `GET` | `/ns/<ns>/ai/models` | 列出有界的已配置 provider model metadata；provider 读取会单独报告 credential status。 |
...
- Deploy 解析支持的 Wrangler/JSONC 形状，校验 bindings 和 routes，通过 `worker:<ns>:<worker>:next_version` 分配下一个 immutable version，并把 bundle metadata/modules/assets 写成 inactive version。Explicit promotion 是独立请求，也是唯一的 active-route flip。分配 version 前，deploy 会按 workerd 64 MiB 上限估算最终 WorkerCode，包含 runtime/do-runtime 注入的 wrapper/client modules 和 workflow import rewrite。Materialized D1 id、DO storage id、Workflow key 与 version identity 位于 host props，不进入 generated source，因此 WATCH retry 内无需再次估算代码。真正的 Redis WATCH commit 会在分配真实 version 并完成 metadata materialization 后执行权威的 headroomed `workerLoader` env-budget 检查，然后才写入 version。
- Promote 是唯一 active-route flip。它会 WATCH 本次候选需要的 delete lock、bundle metadata、D1 ref、service-binding target ref、queue consumer key、host declaration 和 pattern key。EXEC 在一个受审计的 transition 中更新 active route、host 反向索引、cron/queue projection、lifecycle index 和 invalidation publication，并与 route 一起提交 active session policy projection。新的 `restart` sequence 及其 `session-policy:restart` publication 也属于同一个 transaction；重复 promote 同一个 version 会复用已分配 sequence，只 deploy 不 promote 不会分配。不存在 post-commit owner fanout、Control-to-DO backend binding 或 committed-partial response。
...
- AI provider metadata 与 credential 有意分成两次写。替换 metadata 会分配新 revision。同 kind 更新会保留 credential，因为 provider kind 拥有官方 destination 与 bearer authentication shape；kind 变化或在 credential-only residue 上创建 metadata 时，会在同一个 watched transaction 中清除 credential。Credential PUT 必须携带当前 revision，防止延迟写入挂到另一个 provider incarnation。Provider record 是 canonical JSON，credential 使用共享 `WDL-ENC:` envelope，malformed persisted state fail closed。
- Version delete 和 whole-worker delete 都 fail-closed。提交 Redis lifecycle deletion 前，会先收集 active route、retained version、service ref、D1 ref、workflow lifecycle check、queue/cron projection 和 delete lock blocker。成功 whole-worker delete 会在该 commit 内 publish 精确的非持久 `worker:delete` Gateway hint；Gateway 重读 Redis 后才关闭匹配的公开 WebSocket。S3 object cleanup 只在 Redis commit 成功后 enqueue。
...
- `/whoami` 是唯一 namespace-less non-ops action。它对任何有效 token 开放，因为只报告当前 token 自己的 principal、token id、request id 和 public diagnostics。`platformVersion` 是根 `VERSION` 文件中的精确 project release，包含用于区分同一 workerd date 上 WDL-only patch 的末尾 counter；raw workerd version 和 image revision 不返回。`minCliVersion` 声明完整文档化 management surface 所支持的最老下游 CLI，它是 diagnostics metadata，不是 Control-side request enforcement。`urls.control` 是到达 control 的 public origin；当 ingress 提供单个 `x-forwarded-proto` 且值为 `http` 或 `https` 时，`/whoami` 会用该协议生成 `urls.control` 和 `urls.namespace`，否则回退到 control 看到的 request URL 协议。`urls.namespace` 只在 caller 是 tenant namespace token 且显式配置了 `PLATFORM_DOMAIN` 时返回；该门槛只校验 DNS 语法，任何语法合法的 hostname 都会通过，是否公网可达由运维负责。Control 绝不把未配置时的 `workers.local` fallback 伪装成 public URL。`urls.assets` 只在 `ASSETS_CDN_BASE` 是安全的绝对 `http`/`https` URL 时返回，并且会去掉 query 和 fragment。它不能扩展成 token list、token lookup 或携带 secret 的 diagnostics。
...
| `worker:session-policy:<ns>:<worker>` | String | Control | 与 route flip 同事务提交的 active session policy `{version,mode,restartSequence}` projection。 | Whole-worker delete 删除；缺失 state 默认 `preserve`。 |
| `worker:session-policy-seq:<ns>:<worker>` | String counter | Control | 永久 monotonic session-policy restart-event allocator。 | Whole-worker delete 后仍保留，确保重建后的下一次 restart 不会复用 stale Gateway session 已观察过的 sequence。 |
...
| `ai:providers:<ns>` | Hash | Control | 以 provider alias 为 field 的 canonical provider metadata。 | 活过 zero Workers；显式 provider delete 删除对应 field。 |
| `ai:provider-credentials:<ns>` | Hash | Control | 以 provider alias 为 field 的 `WDL-ENC:` provider credential。 | redis-proxy 只为 authenticated AI resolve 解密；显式 provider delete 删除对应 field。 |
