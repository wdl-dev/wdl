...
- 如果 worker-secret PUT/DELETE 已经提交，但后续 version bump 命中 env budget check，Control 会先尝试把 secret hash 回滚，再返回原 budget error。若无法确认回滚成功，Control 返回 `503 secret_mutation_rollback_failed`，并在 details 里带原始 `budget_error`；operator 必须重试 mutation 或修复该 secret 后再 rollout，因为新 secret 可能已经持久化。
