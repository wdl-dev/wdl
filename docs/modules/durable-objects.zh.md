...
DO fetch 和 WebSocket-connect 的 request metadata 最多接受 128 个 header entry，header name 与 value 的 UTF-8 字节合计限制为 128 KiB，不设置更小的单 value 上限。Header name 仍限制为 128 bytes 且必须符合 HTTP token grammar；value 必须是无控制字符的 string。完整转发 URL 限制为 16 KiB UTF-8 bytes。这些是逻辑 metadata 预算，不是端到端 wire 保证：WebSocket 转发会增加内部路由/认证 header，workerd HTTP parser 还会计入 request line，中间代理也可能采用更小的 header 或 URL 上限。接近上限的请求需要部署层预留空间；编码进 header 的大消息（包括 MCP bridge payload）同样占用这份总预算。
...
SQLite `DEFAULT` expression 在声明和使用时遵循 workerd 既有的 function authorization，包括 retained schema。使用 `sqlite_version()` 等未获授权函数的 default 可能在使用时失败；`CURRENT_TIMESTAMP` 仍受支持。该检查不受 compatibility date 或 autogate 门控。WDL 不为此扫描或迁移 schema，也不提供 legacy authorization bypass。
...
- `WEBSOCKET_RECONNECT_DELAYS_MS` 和 `WEBSOCKET_MAX_BUFFERED_MESSAGES` 可以在不改代码的情况下调整 gateway backend reconnect budget 和排队的 client-message 数量上限。每条连接的 queued payload-byte cap 固定为 32 MiB，不可配置。
- Alarm delivery 是 at-least-once。Scheduler 唤醒 Workflows；Workflows 把到期 internal alarm job promote 到 ready，在 DB 2 run token 下 claim，然后调用 do-runtime `/internal/do/alarms/dispatch`。do-runtime 构造 `DoInvoke{kind:"alarm"}` 请求，并走正常 owner router/fence 路径。Delivery correctness 不依赖 HTTP caller-disconnect signal；dispatch timeout 和 Workflows-owned claim lease 约束 unknown result。
- Alarm mutation 使用一个 5 秒 fetch/body deadline。Alarm delivery 的 tenant execution 继续由 Workflows dispatch timeout 与 claim lease 约束，返回 body 再使用独立的 5 秒 deadline；body read 不依赖 best-effort stream cancellation 即可 reject。Response 上限为 16 KiB，并使用 strict UTF-8 JSON variant。do-runtime 只接受 shim 的精确 `{ok:true}` 或 `{ok:true,ignored:true}` actor result，然后保留既有 `{ok:true,ignored:boolean}` Workflows wire；Workflows 验证该 outer shape 后才会 finalize 已 claim job。Mutation success 必须精确带类型正确的 `ok`、`jobId`、`changed` 与 `deleted` 字段：set/delete 要求非空 job id，whole-worker cleanup 要求 `jobId:null`。2xx response 无法读取或格式非法时会 reject，进入既有 operation-specific compensation 与 unknown-outcome contract。
...
- 只有 Workflows request 到达 do-runtime 前的 connect failure，或可信且完整的 `do_alarm_dispatch_failed` response 会立即调度 alarm retry。task/owner resolution 在 owner forwarding 或 actor fetch 开始前失败时，do-runtime 会生成该 failed response；可信 owner response 证明 alarm 已经以失败 settle 时也会生成该 response。pre-dispatch 转换会记录规范化 alarm identity 和原始内部错误，但不会记录 fence token。Timeout、明确的 `do_alarm_dispatch_result_unknown`、owner-forward transport failure，以及无法完整读取、分类的 response 都会让 backend 保留 running claim 到 `WORKFLOWS_DO_ALARM_CLAIM_LEASE_MS` 过期。Running job 会从 ready 移除，并按 lease expiry 写入 due，避免 unknown-result 期间被后续 tick 反复抽取。默认 lease 是五分钟，且配置值会被 clamp 到高于 `WORKFLOWS_DISPATCH_TIMEOUT_MS`，这样 unknown-result 处理可以避免 do-runtime 仍在执行原 dispatch 时并发执行重叠 alarm body。Operator 应按最长预期 alarm handler body 配置 claim lease，而不只是按 HTTP dispatch timeout；alarm body 仍是 at-least-once，claim lease 过期后可能重叠执行。
