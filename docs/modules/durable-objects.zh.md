...
| `worker:do-rollout:<ns>:<worker>` | String | Control | 与 route flip 同事务提交的 active JSON projection `{version,mode,restartSequence}`。 | Whole-worker delete 删除；缺失 projection 视为默认 `preserve`。 |
| `worker:do-rollout-seq:<ns>:<worker>` | String | Control | restart rollout event 的永久单调序号分配器。 | Whole-worker delete 后仍保留，确保重建后的下一次 restart 不会复用旧 Gateway session 已观察过的 sequence。 |
...
- Facet identity 是 stable `doStorageId` 内的 `className:objectName`，因此所有 rollout mode 都保留 SQLite object state。
- Bundle metadata 持有 `durableObjectRollout`。字段缺失或显式 `preserve` 时保持既有行为：已构造的 native facet 会保留 class version，直到 host actor restart 或 facet deletion。`restart` 必须显式 opt in，并要求至少一个 DO binding。
- Promote 在一个 Redis transaction 中提交 active route 和 `{version,mode,restartSequence}` projection。真正的 `restart` promotion 会分配永久 monotonic sequence；重复 promote 同一个 version 会复用该 sequence。只上传 immutable version、不 promote 时不会触发 rollout。
- Route/projection commit 是 rollout linearization point。新的 `restart` sequence 会在同一个 transaction 中 publish 到 `do-rollout:restart`。Gateway 只把事件作为快速 reconciliation hint；是否以 `1012` 关闭较旧的公开 WebSocket session 由最新 Redis projection 决定，subscriber reconnect 也执行同一权威 reconciliation。
- 最新 active projection 生效。后续 `preserve` promotion 会保留已分配 sequence，并覆盖 Gateway、Workflows 或 host actor 尚未观察到的 lazy restart；它不能撤销已经发生的 `1012` 关闭或 facet abort。
...
- 已经运行的 call 和未被 Gateway 持有的既有 facet 不会在 promotion 时被同步枚举或中断；它们会在下一次 dispatch 时收敛。公开 Gateway session 会收到及时 reconciliation hint，但只有最新权威状态仍要求 restart 时才关闭；后续 `preserve` 可以覆盖尚未观察到的 restart。重复 promote 同一个 version 会复用 sequence，不会创建新的 rollout event。
...
- `preserve` mode 下，WDL 会尽量让 client-facing WebSocket 由 gateway 持有并保持连接。健康的旧版本 backend 可以自然 drain；user-runtime 或 do-runtime restart 后，只有该 immutable version 仍 active 时才会透明 backend reconnect。Active version 已变化，或 `restart` mode 下 Gateway session sequence 已过期时，公开连接会以 `1012` 关闭；client 必须重连并重新执行应用握手。Gateway 只在 initial connection race 和异常 backend loss 时读取 atomic route/rollout snapshot，不会逐帧查 Redis。Gateway 重置 backend reconnect epoch 时，旧 epoch 下排队的 client message 可能被丢弃，且没有逐帧 ack/nack。
...
2. Owner resolution 会 WATCH owner record、generation key、worker delete lock、active worker storage pointer 和 active rollout projection。`whole` delete lock 会拒绝 ownership；`version` lock 仍属于 WATCH snapshot，但不会中断 active storage。`restart` projection 会拒绝旧 immutable version，并把 sequence 传给 target-version dispatch。这个 WATCH 会阻止 claim 在 whole-worker delete 或 rollout state 变化后提交。Renew 会先 pipeline 读取 owner/storage snapshot，再通过 Lua CAS 原子比较 owner 原始字节与 active storage pointer，然后刷新 TTL；generation fence 已包含在 owner record 中，不需要再次读取 generation key。
...
do-runtime 围绕 owner resolution、rollout fence、lazy facet restart、dispatch、alarm execution、drain、renew 和 WebSocket 处理输出结构化日志。Workflows 通过 `do_alarm_dispatches` 输出 backend alarm delivery/retry/discard/in-flight-unknown outcome；do-runtime metrics 覆盖 runtime operation。Gateway request log 不衡量 initial 101 之后的 backend WebSocket recovery 生命周期。

## 部署 / Rollout 注意事项

- 先部署 Gateway、Workflows 和 do-runtime projection reader，再部署 system-runtime/Control writer。system-runtime rolling 期间暂停 Control mutation，待该 tier 稳定后再恢复，并且只能在此后允许 API client 发送 `durableObjectRollout`。旧 Gateway、Workflows 或 do-runtime tier 仍在服务时不得启用 `restart`。
...
- Gateway-proxied WebSocket recovery 在 pinned immutable version 仍 active 时，才会在 `preserve` mode 下对 client connection continuity 做 best-effort；backend 脱离后不会重新加载已非 active 的版本。精确 rollout 和 whole-worker-delete hint 只请求对应 worker 的权威检查；当前 projection 为 `restart` 或 worker 已 inactive 时，旧公开 Gateway session 会以 `1012` 关闭，restart 还会在 stale DO facet 下一次 dispatch 时 lazy abort。如果同名 worker 在 delete hint 读到 inactive snapshot 前已经重建，最新 projection 生效；该非持久 hint 不含可识别 delete 前 session 的 incarnation fence。Backend DO facet 在初始 `101` 之后不会逐消息 re-fence；owner handoff 安全依赖 reconnect/rebind 行为，以及创建 backend facet 前运行的 owner-side dispatch fence。Gateway 重置 backend reconnect epoch 时，旧 epoch 下排队的 client message 可能被丢弃，且没有逐帧 ack/nack。
