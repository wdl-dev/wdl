...
Workflows schema-3 maintenance reset 不会枚举或删除 SQLite alarm row。由 Workflows 拥有的 `/workflows schema3-reset` operator command 会 archive schema-2 DB 2，并在恢复 alarm mutation 或 Scheduler dispatch 前把格式未变化的 `wf:internal:do-alarm:*` scheduling projection 复制到 schema-3 DB 2，因此现有 SQLite row token 会继续 fence 复制后的 job。已经因 result-unknown mutation 只留在本地的 row 仍不属于 confirmed delivery contract，并保留 reset 前相同的 best-effort `getAlarm()` repair 行为。最终的 `archive_pending` gate 会阻止普通 Workflow state/lifecycle operation，但继续允许 DO alarm mutation、dispatch 和 tick。
