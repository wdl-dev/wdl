...
- Runtime replay cache 只是 advisory。DB 2 step state 是权威。同一个 runtime isolate 可以在同一 instance incarnation 的不同 run claim 间复用 terminal step record；新 claim 会重新开放有界分页，以发现其他 isolate 已提交的 record。
