...
| `wf:schema3-migration` | String | workflows operator/service | configured Workflows endpoint DB 0 中的 migration ownership 与 completion。 | `in_progress:<token>` 由单个 operator task 通过 CAS 持有并阻止启动；`complete` 允许正常服务，显式删除 archive 后仍保留。 |
...
- Bundled Workers types 暴露 best-effort `locationHint` create option，但 WDL 没有 regional Workflow placement plane；`create()` 与 `createBatch()` 会在 backend I/O 前拒绝传入的自有或继承 `locationHint` field，且不会读取 getter 值。
...
- Schema 3 使用离线保留式迁移。暂停 Workflow create、mutation、execution、worker/version deletion 以及 Durable Object alarm mutation；停止 Scheduler，在旧 Workflows 与 do-runtime 仍可用时让已 admission 的 alarm delivery settle 或等待 claim lease 过期。随后停止并清退全部旧 Workflows dispatch（包括 Runtime 中的 root invocation）；仅停止 sender 不会取消远端执行。禁止部署系统重启旧 release；迁移与 participant 升级都完成前保持相关 surface 暂停。全平台停机是可选简化，不是唯一方式。
- 使用最终 image、Workflows 的 Redis environment 及访问 Workflows/Control endpoint 的 network identity，运行一次性 `/workflows schema3-migrate check`。初次迁移要求专用 schema-2 DB 2、空 DB 15、没有未过期或非法的 running alarm lease，且没有 Redis key TTL。等待 `wf:pending-version:*` 与 internal alarm cleanup snapshot 两类 60 秒临时 key 消退；retention、run/pending-create lease、sleep/retry/alarm deadline 都是 field 或 score，不是 TTL。预检还校验各 key family 的 Redis type、instance identity、payload accounting/reference、step history 以及 pinned Worker export/definition。Payload 校验与 step 转换逐项释放解析后的 JSON；跨 step 检查只保留 dependency metadata，不保留 output/error tree。损坏或有歧义的记录在初次 SWAP 前拒绝；不会猜测 kind 或执行 tenant code 来判定。转换后的 instance 仍必须符合已有 16 MiB aggregate payload limit。
- 保持相关 writer 停止后运行 `/workflows schema3-migrate apply`。它在 Workflows endpoint DB 0 的 `wf:schema3-migration` 获取单写者 ownership，用 `SWAPDB 2 15` 原子归档 DB 2，回拷数据、转换 step、验证 destination，最后发布 schema marker `3`，再将 coordination 从 `in_progress:<token>` 改成 `complete`。迁移未完成时 Workflows 拒绝启动；并发 `apply` 失败。确认失败进程已退出后，用 `/workflows schema3-migrate resume` 通过 exact-value CAS 接管，从 immutable archive 重做未完成工作。若 marker `3` 已发布则只完成 coordination，不再回拷。完成后的 `apply` 为 no-op，`resume` 拒绝执行。
- 转换保留 ID、generation、pinned version、params、terminal output/error、payload、dependency edge、event、summary 和 lifecycle/retention index。Kind 由旧 record shape、host payload reference 和 event consumption 共同判断，不只依赖 tenant config 名称。已有 step 字段值保留原始 JSON 文本，包括浮点 output/error 与 config string；转换只追加 `kind`。Completed step 重放不重新执行 callback，未完成 root work 保持 at-least-once 语义；已 drain 的 running instance 清除旧 run lease 并变为 queued。Waiting 保留绝对 deadline 和 buffered event，paused 保持暂停，terminal 不重新执行，pending-create 不会被提升为已提交实例。DO alarm projection 与 SQLite row 保留原 token；工具不迁移 SQLite。
- 配置最终 Workflows、user-runtime、system-runtime、do-runtime revision 并清退旧参与方，恢复容量前将最终 immutable image 写入部署系统 desired state。迁移报告 `complete` 后，先启动最终 Workflows，再启动/更新需要在启动时解析其 backend address 的 Runtime participant，随后启动 Scheduler，最后恢复相关 surface，禁止新旧 overlap。普通 Workflow API 与 deletion check 立即恢复，由 DB 2 identity/referrer 执行正常约束；保留 DB 15 不阻断服务。具体 stop/update/start 命令由部署系统决定。
- DB 15 默认保持 immutable/inactive，包括迁移成功后。只有显式 `apply --delete-archive` 或 `resume --delete-archive`，且迁移已验证完成时，才删除 DB 15。该 flag 可用于初次迁移，也可在之后通过 no-op `apply` 单独执行，绝不会把旧数据重新覆盖到 live DB 2；`check` 不接受该 flag。删除使用 `FLUSHDB ASYNC`：key 立即消失，Valkey 在后台释放内存，因此命令后的 memory snapshot 仍可能包含尚未释放的部分。DB 0 completion record 应保留。Archive 只是时间点 source，不会回滚之后的 Workflow/KV/D1/DO side effect；外部 snapshot 也不代替用户的删除选择。
- JSON report 提供 instance/step 数、COPY memory estimate、命令前后 memory、`maxMemoryPolicy` 和 advisory warning。估算缺失、eviction policy 或余量不足都不构成容量限制，由 operator 决定。DB 15 与 active DB 共享 memory/eviction，保留期间应持续监测。每次一个 COPY 仍有单个大 value 的 O(N) 阻塞；共享 Valkey endpoint 的低流量 maintenance 接受无关 Control/KV/Queue 短暂停顿，不能接受时应全停。独立 Workflows endpoint 才提供隔离。Control DB 0 definition/bundle 仅被读取，不迁移。
- 独立确认专用 DB 2 数据可丢弃时，可清空后 greenfield 启动而不迁移；这只替代 `check|apply|resume`，不替代 quiescence、旧参与方 drain 和最终启动顺序。清空 DB 2 不会删除 SQLite alarm row，后续 `getAlarm()` 仍可能修复 projection。工具不会自行推断数据可丢弃。
- 遗留部署如果使用了任何非 DB 2 的 Workflows 数据库，必须把本次升级视为 configuration migration；该工具仅接受 schema-2 DB 2 来源。应先删除非 DB 2 的 `WORKFLOWS_REDIS_DB` override；旧数据库若与其它 owner 共享则绝不能清空。只有原 endpoint 的 DB 2 为空且由 Workflows 独占时才能直接使用，否则应把 `WORKFLOWS_REDIS_URL` 指向一个 DB 2 为空的新 endpoint。URL 应省略 database 或显式选择 `2`；显式非 DB 2 URL state 会被拒绝，而不是被静默丢下。
- 只包含 schema-2 Workflows runtime state 的专用 DB 2 是支持的 migration 来源。包含其它 subsystem 所有 key 的共享 DB 2 属于不支持配置：不得做 prefix cleanup 或清空该库，必须把 Workflows 切换到一个 DB 2 为空的新 endpoint。非空但缺少 `wf:schema_version` 的 DB 2 会拒绝启动；只有 operator 确认该库由 Workflows 独占且可以丢弃后才能清空。WDL workflow definitions 位于 DB 0 的 `wf:defs:*`，不属于 DB 2 cleanup。
...
- 不支持 instance event subscription（`subscribe()`）、rollback API 和从指定 step 重启（`restart({ from: ... })`）。Terminate options 中自有或继承的 `rollback` 字段和 restart options 中自有或继承的 `from` 字段会按存在性在 backend call 前被拒绝，不读取 getter 值，即使值是 `false` 或 `undefined`；省略 options 或 options 不包含这些字段时仍执行普通操作。两种 `step.do` overload 都会在读取 replay、claim 或执行 callback 前拒绝传入的 rollback options。Wrapper 只向 host 传递存在性标记，不传递 tenant rollback 对象或函数，因此序列化 hook 无法绕过 host 的 terminal `workflow_invalid_step` 拒绝，即使 tenant 捕获错误也一样。
