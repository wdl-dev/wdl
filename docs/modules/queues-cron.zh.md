...
5. Delayed loop 由 `queue-delayed-wake` 和下一条 due member 的 wall-clock sleep 唤醒。每次 pass 会用有界 pipeline 读取权威 consumer projection 以及 delayed head 或 due range，只有实际存在可处理 member 的 queue 才进入逐 key mutation path。每个到期 member 会先取得 `queue-delayed-claim:*` lease，TTL 为 `SCHEDULER_FIRE_TIMEOUT_MS + 5000ms`；抢到的副本把它移回 main stream，或在 consumer 已消失时移入 orphan stream。
