...
因此 cron 语义是 Cloudflare 风格的 best-effort scheduled event：分钟对齐、不 catch-up replay、允许 overlap，用户 handler 失败只作为 outcome 上报，不由 scheduler 重试。Scheduler 会在 JSON parsing 前以固定 256 KiB body limit 流式读取 cron 和 Queue Runtime response；超限 response 会成为 cron dispatch error 或 Queue retry-all transport failure，但已经取得的 HTTP 413 仍是权威状态，即使 diagnostic body 无法保留，多消息 Queue batch 仍拆分，单消息仍按终态处理。Tenant error text 不能让 Scheduler 无界缓冲 response。
...
2. Scheduler 会在启动时及每个 `SCHEDULER_SWEEP_MS`（默认五分钟）从权威 hash、stream 和 delayed ZSET 修复 `queue:index:*` discovery set。1.5 秒一次的 reconcile loop 只读取这些 index、删除 stale member、为新注册 stream 创建固定的 `wdl-scheduler` consumer group，并维护内存中的 known delayed queue 集合；已注册 stream 不再重复执行 `XGROUP CREATE`，consume loop 观察到 `NOGROUP` 时会在 retry 前同步强制修复 group。正常写入仍由 writer 维护 index；周期 repair 为中断的 data-plus-index 写入提供有界恢复，同时避免在 reconcile 热路径扫描 keyspace。缺失的 optional consumer field 默认使用 `max_batch_size=10`、`max_batch_timeout_ms=5000`、`max_retries=3`、`retry_delay_secs=0` 且不配置 `dead_letter_queue`；已存在但 malformed/out-of-range 的字段或非法 dead-letter queue 会让该 consumer projection 不可用。Group creation 按 stream 隔离错误并继续后续有界 chunk；健康 consumer 会先写入内存 registry 并刷新派生 snapshot，最后才汇报聚合 reconcile failure。
3. Consume loop 使用 `XREADGROUP` 读取 main stream，并按 `max_batch_size` 组 batch 投递；该值必须在 `[1, 100]` 内，越界 projection 会被拒绝。共享 readiness read 的 `COUNT` 取 active stream set 中当前 consumer batch-size snapshot 的最小值。填满该共享 probe 的 stream 可以再执行一次 non-blocking 单 stream read，以填充自身更大的 cap；每个 Scheduler replica 的这些可选 top-up 共用 100-entry 在途预算，预算不可用时不会延迟 readiness probe 已返回的 entry。Redis 对每个 stream 分别应用 `COUNT`，因此基础 readiness reply 仍会随同时非空的 stream 数量增长；top-up 预算不约束这部分回复。每个 Scheduler replica 对同一 stream 最多保持一个 batch 在途，但会继续在共享 queue semaphore 下轮询和派发其他 stream，不等待慢 runtime handler 完成。有 stream 在途时，其余 stream 使用 non-blocking read；空结果会等待 dispatch completion 或最多 100ms 的有界 poll，然后重建 stream set。PEL reap 在 consumer 仍存在时使用同一 per-consumer cap；consumer 已消失的 orphan movement 仍可按 hard cap 分页。Consume 和 PEL reap 可以在 queue semaphore 下按 stream 并行 dispatch。每条 dispatch path 在向 runtime 发送 message 前都会重读该 stream 的权威 `queue-consumer` hash 并刷新内存 registry，因此 consumer promote 后，一旦 message 被选中，就不需要等下一次 reconcile tick 才使用新版本。当前模型里 `max_batch_timeout_ms` 不是 batching wait window。
4. Runtime 返回 queue outcome envelope。Scheduler 解析 explicit `ack`、explicit `retry`、batch retry 和 implicit ack。Retry 与 DLQ transition 会作为彼此独立的原子脚本放在同一 transport pipeline 中执行：所有 target 写入成功后才会 acknowledge 并删除 source entry。因此 target 侧错误会保留 source entry，也不会阻止同 batch 后续健康 transition 完成。不会因为重试同一批 bytes 变好的平台失败，例如 queue message decode failure 或非法 queue dispatch body，会直接进 DLQ，不消耗 retry budget。聚合 request-body-too-large 会先拆成更小 batch 重试。只有成功 HTTP response、outer `outcome: "ok"` 和 object `result` 才能进入 ack/retry resolution；除上述指定永久 400 分类以及 413 的拆分或终态处理外，其余非成功 status、缺失或未知 outer outcome、非 object result 或未知 inner outcome 都会重试完整 batch，不会把无法识别的协议状态当成 implicit ack。执行任何破坏性 acknowledge 前，Scheduler 要求完整 workerd result，包括 inner `outcome`、`ackAll`、`explicitAcks`、`retryMessages` 和 `retryBatch.retry`；它会验证 nested id、boolean 和 `[0, 86400]` 范围内的 integer delay，并要求每个 decision id 都属于当前 batch。缺失或 malformed decision，包括同一 message 同时出现在 explicit ack 与 retry set，都会让完整 batch 重试。
5. Delayed loop 由 `queue-delayed-wake` 和下一条 due member 的 wall-clock sleep 唤醒。每次 pass 会用有界 pipeline 读取权威 consumer projection 和轻量 delayed head score；每个 128-key chunk 中的 eligible queue 共享一次 pipelined member read，并按 `QUEUE_SWEEP_BATCH_SIZE` 分配读取额度、为每个 eligible queue 至少保留一条，同时把浅队列未使用的额度重新分配给同一 chunk。默认每个 chunk 因此最多物化 128 条 member。当前 chunk 会在加载下一 chunk 前完成 claim/mutation，所以完整 message body 保持有界，active consumer 的 member read 也不会退化为每个 queue 一个 RTT。批量 consumer projection 缺失的 queue 属于 destructive orphan 候选；Scheduler 会在 claim 前逐 queue 重读权威 projection，若 consumer 已出现则保留 delayed member。每个到期 member 会先取得 `queue-delayed-claim:*` lease，TTL 为 `SCHEDULER_FIRE_TIMEOUT_MS + 5000ms`；抢到的副本把它移回 main stream，或在 consumer 已消失时移入 orphan stream。
6. Orphan / Pending-Entry cleanup 是诊断和保护机制。它防止 consumer 删除或 scheduler crash 路径静默丢消息，但 main queue stream 仍是 durable backlog，并且故意不 trim。单个 Delayed ZSET mutation 和 orphan stream-tail mutation 受 `QUEUE_SWEEP_BATCH_SIZE` 分页控制，默认 `100`；这与上一条跨 queue member read 的额度是两个不同边界。
...
- Reconcile、`XREADGROUP`、delayed sweep、PEL reap、DLQ、orphan movement 相关日志，包括 `queue_batch_dispatched`、`queue_dispatch_transport_error`、`queue_dispatch_handler_error` 和 `queue_consumer_projection_invalid`。Handler warning 会携带有界的 fail-closed decision reason，但不会把它加入 metric label。
