...
- `test-workers/` 保存 integration tests 直接部署、读取或作为 hermetic workerd service 嵌入的 fixture。三种结构并存且有意：完整 Wrangler workspace、source-only worker，以及带 `config.capnp` 的 embedded service。按用法挑选；详见 `test-workers/README.zh.md`。`ai-binding`、`ai-openai-sdk` 和 `ai-provider` fixtures 分别拥有 tenant facade、SDK compatibility 和本地 official-provider protocol simulation。
