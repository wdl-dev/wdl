...
| Python Workers modules | Not supported | workerd 已有 release-gated Python Workers 路径；`python_workers_20260610` 不再是 experimental，并且只在 compatibility date `2026-08-25` 之后 implied。 | Control 用 `python_workers_unsupported` 拒绝 `py` module manifest；runtime 和 do-runtime 对 retained metadata 中的 `py` module 也 fail closed。 | WDL 保持 tenant bundle 只包含 JavaScript/WebAssembly/data，不允许 cold-load 时触发 Pyodide bootstrap。 | 不支持 Python Workers 和 JS/Python 混合 bundle。 |
...
Bundled 2026-08-13 workerd pin 支持 removal signal 就是其自身 `EventTarget` 的 listener，并在 abort 驱动的 `ReadableStream.pipeTo()` teardown 中保留 replacement reader lock。Blob stream 和 Blob-backed response 会在 sandbox boundary 复制 bytes，workerd native writable-sink adapter 也会在异步 native I/O 前复制 JavaScript chunk；这些路径执行 O(n) safety copy。WDL 的 direct fixed `ArrayBuffer` R2 PUT normalization 仍为 zero-copy。Buffered R2 stream consumer 会在 chunk 交付给自身 reader 后、继续下一次 read 前取得 snapshot；`ReadableStreamDefaultController.enqueue()` 仍采用 reference semantics，因此 producer 在 enqueue 后的 mutation 可能在该 snapshot 前改变 delivered bytes。自有 Queue binding 不使用 workerd native Queue producer。HTMLRewriter 使用 lol-html 3.0.1，并包含修正后的 selector 与 SVG/MathML handling。这些 binary behavior 不受 compatibility date gate 控制。
...
| Vars 和 secrets | Supported | Env values 可 materialize 到 worker `env`。 | Control 存 worker vars/secrets metadata，把 secret value 加密成 at-rest `WDL-ENC:` envelope；redis-proxy 在 cold-load 时解密；deploy/secret mutation 过程中按留有 headroom 的 workerd 1 MiB `workerLoader` serialized env budget 校验用户 vars/secrets 加 runtime 注入的 binding env value，并计入非 Latin-1 字符串的 V8 two-byte storage；worker secret 改变时 promote immutable version。Workflow identity 留在生成的 wrapper code 中，并由 WorkerCode budget 计数。 | Secrets 由 WDL 平台管理；at-rest envelope provider 是 WDL 部署关注点。 | 不是 Cloudflare account secrets。 |
