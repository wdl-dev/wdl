...
| `compatibility_date` / `compatibility_flags` | Partial | capnp 中启用的 workerd feature flags 和兼容行为。 | Dynamic Worker 会拒绝早于 `2026-04-01` 的显式 `compatibility_date`；Control 还会校验该值是真实的 `YYYY-MM-DD` 日期，且不晚于当前 UTC 日期和当前 bundled workerd 支持的最大日期，并拒绝上游 `$experimental` enable flags（例如 `experimental` / `unsafe_module`）、`legacy_error_serialization`、`allow_irrevocable_stub_storage` 和 `streams_disable_constructors`。冗余或其他不兼容的上游 flag 由 workerd 在 cold load 时校验。 | WDL 把 compatibility metadata 当成部署期平台 metadata，不承诺完整 per-worker 历史行为模拟层。 | 没有 per-worker 模拟所有 Cloudflare 历史行为；tenant worker 不能 opt into 上游 experimental-only flags、禁用 WDL 要求的 enhanced error serialization 或标准 stream constructors，或持久化不可撤销的 capability stub。 |
...
| 上游 experimental 和 WDL 显式拒绝的 compatibility flags | Not supported | Tenant `compatibility_flags` 中属于上游 workerd `$experimental` 的 enable flag，以及 WDL 显式拒绝的 `allow_irrevocable_stub_storage` 和 `streams_disable_constructors`，会在 deploy 和 runtime decode 阶段被拒绝；这既排除了仅在 New Module Registry 下可用的 `import.meta.dirname` 和 `import.meta.filename`，也阻止 tenant worker 禁用 WDL runtime facade 依赖的标准 stream constructor。 |
