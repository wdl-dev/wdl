...
`spec_compliant_dispatch_exceptions` is non-experimental and accepted as an explicit
...
Compatibility flags are part of the architecture toolbox, not a uniformly forbidden
surface. WDL evaluates them by who executes the code and what capability the flag adds:

- Platform-owned static workers should proactively evaluate explicit flags when the
  native workerd behavior removes WDL code, makes a trust boundary smaller, or avoids a
  parallel implementation. Pin the flag in the owning Cap'n Proto service, audit the
  upstream implementation and future compatibility implications, and cover the enabled
  behavior with real-workerd tests.
- Tenant Dynamic Worker flags are intentionally more conservative. Enabling one changes
  a public execution contract across arbitrary tenant code and retained versions. It
  needs an explicit product decision, Control and retained-state policy, interaction
  review with generated wrappers and binding isolation, and a rollout plan where
  required.
- A flag must be judged by its complete capability and trust-zone blast radius, not only
  by whether it makes one call site easier. A platform-only flag may be appropriate even
  when the analogous tenant flag is denied. Conversely, do not expose a broad tenant
  capability merely to solve a narrow host-plumbing problem.
...
in memory. Under the current design, WDL rejects `allow_irrevocable_stub_storage` at
deploy and retained-state load, and static host workers do not enable it, because the
tenant-visible persistent-stub surface is broader than WDL's current binding contracts.
...
| Surface | Status | What workerd provides | Stronger / added in WDL | Different from Cloudflare | Not implemented / gaps |
|---|---|---|---|---|---|
| KV namespace | Supported | A binding object can be exposed to user code through workerd entrypoint/JSRPC mechanics. | Runtime `KV` facade calls redis-proxy; redis-proxy stores values and metadata in DB 1 hash buckets `kvh:<ns>:<id>:b:<bucket>`, with `v:<key>` and `m:<key>` fields, 512-byte key/list-prefix cap, 1024-byte serialized metadata cap, 25 MiB value cap, batch raw-byte budget, TTL/EXAT, and prefix list cursors. | KV storage is Redis-backed and namespace-scoped in a WDL deployment. `cacheTtl` is not a storage freshness contract. | No Cloudflare global edge replication or eventual consistency model. |
...
| Workflows | Partial | User workflow class code runs inside loaded workers when dispatched by runtime. | workflows owns DB 2 instance state, step/event/sleep commits, runtime-observed `step.do` DAG edges including `Promise.all` siblings, generation/run-token fencing, lifecycle APIs, progress callbacks, and scheduler tick. | Payloads are JSON data under WDL byte caps; permanent `step.do` failure is terminal for the run even if caught, and one step may record at most 1000 dependency edges. | Native workerd `WorkflowInstance.delete()`, `Workflow.deleteBatch()`, instance event subscriptions (`subscribe()`), rollback APIs, and restarting from a selected step (`restart({ from: ... })`) are not exposed by WDL's custom facade. Rollback and restart-from options are rejected rather than downgraded to ordinary operations. `locationHint`, `script_name`, cross-worker workflows, and Cloudflare's source-AST visualizer are also unsupported. |
| Service bindings | Supported | workerd service binding JSRPC and fetch dispatch. | WDL resolves target worker metadata, cold-loads immutable versions, propagates request ids where available, and enforces namespace/action ACL through control metadata. | Frozen-version service targets do not evict active siblings by design. | Object-style custom RPC has no general generated env facade; use class-based `WorkerEntrypoint` methods with `this.env` when binding facades are required. |
...
D1 and Durable Object SQL `DEFAULT` expressions obey workerd's existing function
authorization, including defaults in retained schemas. See the
[D1](modules/d1.md#redis--storage-contracts) and
[Durable Objects](modules/durable-objects.md#redis--storage-contracts) storage
contracts.
