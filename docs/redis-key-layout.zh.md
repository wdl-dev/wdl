...
Local compose、Kubernetes 和 Terraform 都启用这个切分。Rust service 和 Rust `redis-proxy` 使用 `DATA_REDIS_URL` 选择完整的 data-plane Redis endpoint 与逻辑数据库；`DATA_REDIS_DB` 不影响这些 Rust client。嵌入的 JS control/log-tail 路径使用 `DATA_REDIS_ADDR` 加 `DATA_REDIS_DB`，因为它们的 RESP client 接收 host:port address。未设置对应路径变量的部署会把 data-plane key 留在 control Redis connection/database，直到显式 opt in。Workflows 不同：`WORKFLOWS_REDIS_URL` 可以选择其它 Redis endpoint，但必须省略 database 或显式选择 DB 2；显式非 DB 2 URL 会被拒绝。DB 2 必须由 Workflows runtime state 独占，不能与 control 或 data-plane state 共享。遗留 `WORKFLOWS_REDIS_DB` 配置只允许值 `2`，并应从 deployment configuration 中删除。Workflows 会比较解析后的 database identity 与 `CONTROL_REDIS_URL`，以及 Rust data plane 的有效 URL（`DATA_REDIS_URL ?? REDIS_URL`）；拆分 data plane 的部署必须把 canonical URL 传给 Workflows 以执行该启动检查。
...
示例省略了 `sessionPolicy`；Control 只为非默认的 `restart` policy 持久化该字段，字段缺失表示 `preserve`。session-policy 改名前持久化的 bundle 可能携带已退役的 `durableObjectRollout` 字段；routing 会把它按同一 mode enum 读取，新 deploy 只写 `sessionPolicy`。secret bump 会原样拷贝源 bundle 的 `__meta__`，因此旧字段可能随拷贝延续到改名后分配的新版本号；双读是长期合同，不是过渡期机制。
...
Routes、platform-domain exposure、crons、queue consumers、session policy mode、bindings、vars、exports、workflow definitions 和 asset prefixes 都是 version metadata。`workersDev: false` 表示显式关闭 platform-domain exposure；字段缺失表示启用。Rollback 本质上是 promote 一个旧的 immutable version。
