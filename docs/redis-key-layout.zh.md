...
- **`DB 0`，控制面：**bundle、routes/patterns、auth、AI provider metadata/credential、D1/DO owner state、cron config、queue-consumer config、lifecycle metadata、workflow definition（`wf:defs:*`），以及 Workflows schema-reset state。
