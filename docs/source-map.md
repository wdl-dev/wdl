...
| `runtime/dispatch.js`, `runtime/dispatch/*`, and `runtime/infrastructure-error.js` | Fetch, scheduled, queue, and workflow dispatch; workflow step facade, replay cache, and deterministic JSON helpers; the host-owned KV read failure code, facade RPC trampoline method, and reporter origin; and the report-only Workflow failure latch. |
| `runtime/load.js` and `runtime/load/*` | Bundle decode, module rewrite, env construction, wrapper generation, injected runtime source ownership, hidden binding stripping, and the root `_wdl-*` reserved-module namespace shared with Control. `module-rewrite.js` owns the private Workflow infrastructure reporter prop; `wrapper-generate.js` owns tenant-inaccessible KV/reporter callable binding, native `RpcPromise` settlement, the explicit Workflow step facade, exact-Error branding, and escape-boundary reporting; `code-budget.js`, `injection-sources.js`, and `wrapper-generate.js` also own generated AI facade inclusion for AI-only bundles. |
| `runtime/load/injection-sources.js` | Canonical tenant-source registry embedded as Cap'n Proto text modules by the three base runtime configs. It owns `runtime/d1-client.js`, `shared/d1-data-field.js`, `shared/d1-params.js`, `shared/utf8.js`, `shared/sql-splitter.js`, `runtime/r2-client.js`, `runtime/r2-utils.js`, `runtime/do-client.js`, `runtime/_wdl-do-scoped-request.js`, `runtime/_wdl-request-id.js`, `runtime/workflows-client.js`, and `runtime/ai-client.js` as generated-wrapper inputs. Host-only D1 compatibility decoding and DO routing remain outside this registry. |
| `runtime/bindings/` | Host-side binding adapters for KV, D1, R2, Durable Objects, Workflows, ASSETS, service, queue, and AI. `runtime/bindings/kv-capacity.js` owns process-local KV response wire-byte admission, deadlines, and scrape-time gauges; `runtime/bindings/do.js` and `runtime/bindings/workflow.js` own binding-scoped identity/authenticated transports; `runtime/bindings/ai.js` owns AI admission, provider resolution, public-only forwarding, and protocol orchestration; `runtime/bindings/ai-capacity.js` owns process-local pool accounting, lease transfer, metrics, deadlines, and watchdog cleanup; `runtime/bindings/ai-provider.js` owns official destinations, provider request policy, credential-bearing headers, and bounded response headers; `runtime/bindings/ai-sse.js` owns incremental SSE framing and terminal-event classification; `runtime/bindings/ai-websocket.js` owns bounded frame forwarding, model rewriting, idle detection, and close propagation. |
| `runtime/do-client.js`, `runtime/_wdl-do-scoped-request.js`, `runtime/bindings/do.js`, `runtime/_wdl-do-transport.js`, `do-runtime/protocol/wire-grammar.js` | Tenant DO namespace/id/stub facade and its binding-scoped request envelope; host-only identity attachment, canonical forwarding and owner-shard headers, owner routing, invoke/connect framing, retries, and response stripping. |
...
| `runtime/bindings/workflow.js`, `runtime/workflows-client.js`, `runtime/dispatch/workflow-*.js`, `runtime/load/wrapper-generate.js` | Workflow binding-scoped identity enforcement and transport, facade materialization, dispatch facade, replay cache, and step semantics. |
...
| `shared/owner-endpoint.js`, `shared/owner-lease.js`, `shared/owner-protocol.js`, `shared/owner-forwarder.js` | Shared owner endpoint grammar, lease parsing, cross-language `*_OWNER_TTL_SECONDS` environment syntax, generation counters, key derivation, fence matching, staged Redis owner writes, and authenticated forwarding mechanics used by Control and the D1/DO runtimes. |
| `shared/auth-roles.js` | Role table, principal validation, reserved namespace policy, and auth action capabilities. |
| `shared/auth-token.js` | Canonical `x-admin-token` sanitizer used by Control before it calls Auth. |
...
| `shared/respond.js` | Shared HTTP response, JSON error, Prometheus text, canonical Content-Type essence parsing, best-effort response body discard, and `x-request-id` echo helpers. |
...
| `shared/do-alarm-response.js` | Shared 5-second body deadline, 16 KiB strict UTF-8 reader, and exact success-variant parser for do-runtime alarm delivery/mutations and Control whole-worker cleanup. |
...
| `shared/env.js` | Empty/nullish environment fallback primitive plus bounded canonical positive-decimal parsing for cross-process runtime configuration readers. |
...
| `types/workerd-node-compat.d.ts`, `types/workerd-node-modules.d.ts` | Narrow type boundary for the Node-compatible globals and imported Node module surface used by workerd product code. |
| `types/node-typecheck-contracts.ts` | Node-only ambient-type tripwire that rejects Workers declarations leaking into the scripts/tests checker. |
...
| `rust/workflows/` | Workflows service, DB 2 state machine, internal DO alarm jobs, and the operator-only schema-3 migration command. `schema_migration.rs` owns offline coordination, copy/verification and optional archive deletion; `api/schema2.rs` owns instance conversion and `api/execution/schema2.rs` owns legacy step-kind conversion. |
