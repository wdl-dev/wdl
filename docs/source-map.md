...
| `runtime/dispatch.js`, `runtime/dispatch/*`, and `runtime/infrastructure-error.js` | Fetch, scheduled, queue, and workflow dispatch; workflow step facade, replay cache, and deterministic JSON helpers; and exact invocation-scoped host-failure attribution used to keep retryable infrastructure failures out of durable step results. |
| `runtime/load.js` and `runtime/load/*` | Bundle decode, module rewrite, env construction, wrapper generation, injected runtime source ownership, hidden binding stripping, and the root `_wdl-*` reserved-module namespace shared with Control. `module-rewrite.js` owns the private Workflow infrastructure invocation prop; `code-budget.js`, `injection-sources.js`, and `wrapper-generate.js` also own generated AI facade inclusion for AI-only bundles. |
...
| `shared/do-alarm-response.js` | Shared 16 KiB strict UTF-8 reader and exact success-variant parser for do-runtime alarm delivery/mutations and Control whole-worker cleanup. |
