...
| `runtime/do-client.js`, `runtime/_wdl-do-scoped-request.js`, `runtime/bindings/do.js`, `runtime/_wdl-do-transport.js`, `do-runtime/protocol/wire-grammar.js` | Tenant DO namespace/id/stub facade and its binding-scoped request envelope; host-only identity attachment, canonical owner-shard hint projection, owner routing, invoke/connect framing, retries, and response stripping. |
...
| `shared/d1-*.js`, `shared/sql-splitter.js` | D1 parameter, data-field, transport, timeout, query-wire/owner-hint/result-header and ownership-code contracts, and SQL splitting utilities shared by runtime, d1-runtime, control, and tests. |
