...
- logging call site、event name 和 service-specific request completion 语义
...
- `wdl-rust-common` 中的 Redis helper 可以使用共享 transport settings 构造 client、从显式 keys/args 构造命令，或针对显式 `ConnectionManager` 运行调用方传入的 closure。script body、pool topology、选择哪条 connection、retry/timeout 行为、error mapping 和 state ownership 仍由 service crate 拥有。
